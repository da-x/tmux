    ...
    u_int px, u_int py, u_int nx, u_int ny)
{
	...
	for (yy = py; yy < py + ny; yy++) {
		...
		cx = s->cx;
		for (xx = px; xx < px + nx; xx++) {
			if (xx >= grid_get_linedata(gd, yy)->cellsize)
				break;
			grid_get_cell(gd, xx, yy, &gc);
			if (xx + gc.data.width > px + nx)
				break;
...
screen_write_backspace(struct screen_write_ctx *ctx)
{
	...
	if (s->cx == 0) {
		if (s->cy == 0)
			return;
		gl = grid_get_linedata(s->grid, s->grid->hsize + s->cy - 1);
...
screen_write_clearline(struct screen_write_ctx *ctx, u_int bg)
{
	...
	gl = grid_get_linedata(s->grid, s->grid->hsize + s->cy);
...
screen_write_linefeed(struct screen_write_ctx *ctx, int wrapped, u_int bg)
{
	...
	gl = grid_get_linedata(gd, gd->hsize + s->cy);
	if (wrapped)
